...
def publish_assignment(assignment_id: int, db: Session = Depends(database.get_db), current_user: models.User = Depends(get_current_teacher)):
    ...
    if assignment.class_id:
        students = db.query(models.Student).filter(models.Student.class_id == assignment.class_id).all()
    elif assignment.grade_id:
        students = db.query(models.Student).filter(
            models.Student.class_id == None,
            models.Student.grade_id == assignment.grade_id
        ).all()
    else:
        students = []
