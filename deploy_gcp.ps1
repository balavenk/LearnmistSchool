...
Invoke-Expression "$gcloudCmd compute firewall-rules create allow-http --allow tcp:80 --target-tags http-server --quiet" 2>$null

Write-Host "Deployment preparation complete!" -ForegroundColor Green
Write-Host "To finish deployment:"
Write-Host "1. SSH into the VM: $gcloudCmd compute ssh $vmName --zone=$zone"
Write-Host "2. Run: mkdir app && tar -xzf app.tar.gz -C app && cd app"
Write-Host "3. Run: docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v `"`$PWD:`$PWD`" -w=`"`$PWD`" docker/compose:1.29.2 up -d"
