...
const SchoolTypes: React.FC = () => {
    ...
    const handleEdit = (item: SchoolType) => {
        setModalMode('edit');
        setCurrentId(item.id);
        setFormData({ name: item.name, country_id: item.country_id.toString() });
        setIsModalOpen(true);
    };
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            ...
            fetchData();
            alert("School Type deleted successfully");
        } catch (error: any) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (modalMode === 'add') {
                ...
                alert("School Type created successfully");
            } else {
                if (!currentId) return;
                await api.put(`/super-admin/master/school-types/${currentId}`, payload);
                alert("School Type updated successfully");
            }
        ...
        <div className="space-y-8">
            ...
            <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl shadow-lg p-8 text-white">
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                    <div>
                        <h1 className="text-2xl font-bold mb-2">School Types</h1>
                        <p className="text-indigo-100 text-lg">Manage school level types by country</p>
                    </div>
                    <button
            ...
            <div className="bg-white rounded-2xl shadow-md border-2 border-slate-200 overflow-hidden">
                <div className="bg-gradient-to-r from-slate-50 to-slate-100 px-6 py-4 border-b-2 border-slate-200">
                    <h3 className="text-lg font-bold text-slate-800">All School Types ({filteredList.length})</h3>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b-2 border-slate-200">
                                <th className="px-6 py-4 text-xs uppercase font-bold text-slate-600 tracking-wider">School Type Name</th>
                        ...
                        {loading ? (
                            ...
                            <tr><td colSpan={3} className="px-6 py-16 text-center">
                                <div className="flex flex-col items-center">
                                    ...
                                    <p className="text-slate-500 font-medium text-lg">No school types found</p>
                                </div>
                            </td></tr>
