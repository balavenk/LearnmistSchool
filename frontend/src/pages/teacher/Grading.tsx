...
const TeacherGrading: React.FC = () => {
    ...
    const studentColumns = useMemo<ColumnDef<Student>[]>(() => [
        {
            accessorKey: 'name',
            header: isCorporate ? 'Employee Info' : 'Student Info',
            cell: ({ row }) => (
        ...
        <div className="space-y-8">
            ...
            <div className="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 rounded-2xl p-6 shadow-sm border border-indigo-100">
                ...
                <p className="text-slate-600 flex items-center gap-2">
                    ...
                    Select a class and subject to view {isCorporate ? 'employees' : 'students'} and manage grades
                </p>
            </div>
                        ...
                        >
                            {fetchingStudents ? (
                                ...
                                <>
                                    ...
                                    Show {isCorporate ? 'Employees' : 'Students'}
                                </>
                            )}
                        </button>
                    </div>
                ...
                <div className="bg-white rounded-2xl shadow-md border-2 border-slate-200 overflow-hidden animate-fade-in-up">
                    <div className="bg-gradient-to-r from-slate-50 to-slate-100 px-6 py-4 border-b-2 border-slate-200 flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            ...
                            <h2 className="text-xl font-bold text-slate-800">Step 3: {isCorporate ? 'Employee Roster' : 'Student List'}</h2>
                        </div>
                        <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-lg shadow-sm border border-slate-200">
                            ...
                            <span className="text-slate-600 font-medium">{isCorporate ? 'Employees' : 'Students'}</span>
                        </div>
                    </div>
                    <div className="overflow-x-auto">
                        <DataTable
                            ...
                            emptyMessage={isCorporate ? "No employees found. Please select a class and subject." : "No students found. Please select a class and subject."}
                        />
                    </div>
                </div>
            )}
            ...
            {!fetchingStudents && students.length === 0 && selectedClassId && selectedSubjectId && (
                <div className="bg-gradient-to-br from-slate-50 to-slate-100 rounded-2xl border-2 border-dashed border-slate-300 py-16 animate-fade-in-up">
                    <div className="text-center">
                        ...
                        <h3 className="text-xl font-bold text-slate-900 mb-2">{isCorporate ? 'No Employees Found' : 'No Students Found'}</h3>
                        <p className="text-slate-500">There are no {isCorporate ? 'employees' : 'students'} enrolled in this class yet.</p>
                    </div>
                </div>
            )}
        </div>
