...
const TeacherGrading: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                console.log('Fetching teacher data (classes, subjects, settings)...');
                const [classesRes, subjectsRes, settingsRes] = await Promise.all([
                    api.get('/teacher/classes/'),
                    api.get('/teacher/subjects/'),
                    api.get('/teacher/settings')
                ]);
                console.log('Received classes:', classesRes.data.length);
                console.log('Received subjects:', subjectsRes.data.length);
                ...
                setError("Failed to load teacher data. Please refresh the page or contact support.");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleShowStudents = async () => {
        ...
        try {
            ...
            console.log('Received students:', response.data.items?.length || 0);
